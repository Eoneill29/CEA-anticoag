...
Testing ICER sensitivity to Probability of Off-treatment to Recurrence (OT > VR)
...
riv.u.ot    <- 0.59                #Utility of off treatment 
##Warfarin Values 
...
war.u.ot    <- 0.59                #Utility of off treatment
...
## Probability of Off-treatment to Recurrence (OT > VR)
#Event rates per person-6months converted to person-year:
riv.r.ot_vr   <- seq(0, 0.074, by=0.001)     # 7.4% annual rate of off-treatment to recurrence
riv.pr.ot_vr <- 1 - exp(-riv.r.ot_vr * cycle_length)
war.r.ot_vr   <- seq(0, 0.074, by=0.001)     # 7.4% annual rate of off-treatment to recurrence
war.pr.ot_vr <- 1 - exp(-war.r.ot_vr * cycle_length)

n_samples <- length(riv.r.ot_vr)
...
riv.input_params <- as.data.frame(cbind(nsim = as.character(seq(1, n_samples)),
                                        ...
                                        pr.ot_vr = as.numeric(riv.pr.ot_vr))) ### SENS
...
war.input_params <- as.data.frame(cbind(nsim = as.character(seq(1, n_samples)),
                                        ...
                                        pr.ot_vr = as.numeric(war.pr.ot_vr))) ### SENS
...
for (i in 1:n_samples){
  sim_markov_riv <- Markov(n.t = riv.input_params$n.t[[i]], 
                           ...
                           pr.ot_vr = riv.input_params$pr.ot_vr[[i]])
...
# Save psa object
saveRDS(psa, here::here("output","results_1yr_sens_04d_ProbRecurrence.rds"))
